...
document.getElementById("loginForm").addEventListener("submit", async (e) => {
  ...
  if (!user || !pass || !file) {
    ...
    setTimeout(
      () => card.classList.replace("translate-x-2", "-translate-x-2"),
      100,
    );
    setTimeout(
      () => card.classList.replace("-translate-x-2", "translate-x-0"),
      200,
    );
    return;
  }
...
window.addEventListener("keydown", (e) => {
  if (e.altKey && window.pywebview) {
    if (e.code === "ArrowLeft") {
      e.preventDefault();
      window.pywebview.api.snap_left();
    } else if (e.code === "ArrowRight") {
      e.preventDefault();
      window.pywebview.api.snap_right();
    } else if (e.code === "ArrowUp") {
      e.preventDefault();
      window.pywebview.api.snap_top();
    } else if (e.code === "ArrowDown") {
      e.preventDefault();
      window.pywebview.api.snap_bottom();
    }
  }
});

...
function renderStudents() {
  ...
  students.forEach((student) => {
    ...
    if (student.grade === "") {
      ...
      if (gradeNum >= 8.0)
        gradeColorClass = "text-brand-600 dark:text-brand-400 font-black";
      else if (gradeNum < 5.0)
        gradeColorClass = "text-red-500 dark:text-red-400 font-bold";
...
function startListening() {
  if (
    isListening ||
    document
      .getElementById("cmdOverlay")
      .classList.contains("modal-overlay-enter")
  )
    return;
  isListening = true;
  ...
  setTimeout(
    () => listeningWaves.classList.replace("scale-50", "scale-100"),
    10,
  );
  setTimeout(
    () => listeningWaves.classList.replace("opacity-0", "opacity-100"),
    10,
  );
...
function toggleTheme() {
  ...
  document
    .getElementById("cmdMoonIcon")
    .setAttribute("data-lucide", isDarkMode ? "sun" : "moon");
  lucide.createIcons();
  renderStudents();
}
...
window.addEventListener("keydown", (e) => {
  if ((e.ctrlKey || e.metaKey) && e.key === "k") {
    e.preventDefault();
    cmdOverlay.classList.contains("hidden")
      ? openModal(cmdOverlay, cmdModal, cmdInput)
...
document
  ...
  .addEventListener("click", () => openModal(cmdOverlay, cmdModal, cmdInput));
cmdOverlay.addEventListener("click", (e) => {
  if (e.target === cmdOverlay) closeModal(cmdOverlay, cmdModal);
});

// LÓGICA DE BÚSQUEDA EN COMMAND PALETTE
const emptySearchMsg = document.createElement("div");
emptySearchMsg.className =
  "px-6 py-10 text-center hidden flex flex-col items-center justify-center text-zinc-400";
